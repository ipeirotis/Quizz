...
public class CampaignManagement extends HttpServlet {
	...
	private void test(HttpServletResponse resp) throws Exception {
		...
		if (page.getEntries() != null) {
			for (Campaign campaign : page.getEntries()) {
				pw.println("<h4>Campaign with name \"" + campaign.getName() + "\" and id \""
						+ campaign.getId() + "\" was found.</h4>");
			}
		} else {
			pw.println("No campaigns were found.");
		}
		pw.flush();
	}
	
	private void addCampaign(HttpServletRequest req, HttpServletResponse resp) throws Exception{
