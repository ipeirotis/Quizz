...
do
   ...
   curl $API_URL/insertQuestion --header 'Content-Type: application/json' --data  '{ "quizID": "'$QUIZ_ID'", "questionText": {"value": "Calibration Question '$i'"}, "kind": "MULTIPLE_CHOICE_CALIBRATION", "weight": 1, "answers": [{ "text": "Answer 1 - gold", "kind": "GOLD" }, { "text": "Answer 2", "kind": "INCORRECT" }, { "text": "Answer 3", "kind": "INCORRECT" }, { "text": "Answer 4", "kind": "INCORRECT" }] }'
done

for i in {1..10}
do
   echo "Adding collection question #$i"
   curl $API_URL/insertQuestion --header 'Content-Type: application/json' --data  '{ "quizID": "'$QUIZ_ID'", "questionText": {"value": "Collection Question '$i'"}, "kind": "MULTIPLE_CHOICE_COLLECTION", "weight": 1, "answers": [{ "text": "Answer 1 - silver", "kind": "SILVER", "probability": 0.75 }, { "text": "Answer 2", "kind": "INCORRECT" }, { "text": "Answer 3", "kind": "INCORRECT" }, { "text": "Answer 4", "kind": "INCORRECT" }] }'
done

echo "Updating the statistics for the Quiz"
curl $API_URL/updateQuizCounts?quizID=$QUIZ_ID
echo "Done..."

#TODO: Check that the updated numbers are ok
# GET https://crowd-power.appspot.com/_ah/api/quizz/v1/getQuiz?id=$QUIZ_ID

# Check that we get back calibration and collection questions
# GET https://crowd-power.appspot.com/_ah/api/quizz/v1/quizquestions/$QUIZ_ID
...
curl -i -H "Accept: application/json" -X DELETE $API_URL/removeQuizRecursively?id=$QUIZ_ID
