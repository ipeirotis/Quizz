...
NUM_QUESTIONS = 10;
CURRENT_QUIZZ = -1;
CURRENT_GOLD_QUIZZ = -1;
CURRENT_SILVER_QUIZZ = -1;
IS_NEXT_GOLD= true;
QUIZZ_GOLD_QUESTIONS = [];
QUIZZ_SILVER_QUESTIONS = [];
USER_ANSWERS = [];
USER_FEEDBACKS = [];
...
function shuffle(array) {
	...
	function initializeQuizzWithQuestions(questions) {
		QUIZZ_GOLD_QUESTIONS = questions['gold'];
		QUIZZ_SILVER_QUESTIONS = questions['silver'];
		presentNextQuestion();
	}
	...
	function presentNextQuestion() {
		...
		$('#questionsPackProgress').html("Question " + (currentQuizzNo) +
		 			" out of " + QUIZZ_GOLD_QUESTIONS.length);
		
		var nextQuestion;
		if(IS_NEXT_GOLD || !QUIZZ_SILVER_QUESTIONS[CURRENT_QUIZZ]){
			nextQuestion = QUIZZ_GOLD_QUESTIONS[CURRENT_QUIZZ];
		}else{
			nextQuestion = QUIZZ_SILVER_QUESTIONS[CURRENT_QUIZZ];
		}
	...
	function prepareNextQuestion() {
		if (CURRENT_QUIZZ === QUIZZ_GOLD_QUESTIONS.length - 1) {
			endOfQuizzPack();
		} else {
			presentNextQuestion();
		}
	...
	function endOfQuizzPack () {
		...
		$('#totalCountSummary').html(QUIZZ_GOLD_QUESTIONS.length);
		updateSharingParameters(correctCount, QUIZZ_GOLD_QUESTIONS.length);
	}
	...
	function answeredQuestion (answer){
		return function () {
			...
			formData.push({'name': 'a', 'value': getNumberOfCorrectAnswers()});
			formData.push({'name': 'b', 'value': NUM_QUESTIONS - getNumberOfCorrectAnswers()});
			formData.push({'name': 'c', 'value': USER_FEEDBACKS.length});
	...
	function getNumberOfCorrectAnswers() {
		var result = 0;
		$.each(USER_FEEDBACKS, function(index, feedback) {
