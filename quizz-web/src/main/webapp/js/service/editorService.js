angular.module('quizz').factory('editorService', ['$http', '$cacheFactory',
  ...
  return {
    ...
    getQuiz: function(quizID, success, error) {
      ...
      if (!fromCache) {
        $http.get(Config.api + '/getQuiz', {'params': {quizID: quizID}})
    ...
    saveQuiz: function(quiz, success, error) {
      $http.post(Config.api + '/insertQuiz', quiz)
          .success(function(savedQuiz) {
    ...
    listQuestions: function(quizId, success, error) {
      $http.get(Config.api + '/listAllQuestions', {'params': {quizID: quizId}})
           .success(success).error(error);
    },

    getQuestion: function(id, success, error) {
      $http.get(Config.api + '/getQuestion', {'params': {id: id}})
           .success(success).error(error);
    },

    saveQuestion: function(question, success, error) {
      $http.post(Config.api + '/insertQuestion', question)
           .success(success).error(error);
    },

    removeQuestion: function(id, success, error) {
      $http['delete'](Config.api + '/removeQuestion/' + id)
          .success(success).error(error);
    }
  };
}]);
