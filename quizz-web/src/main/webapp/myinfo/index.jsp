    ...
    <div class="row">
      <div class="col-sm-offset-2 col-sm-8 col-md-offset-2 col-md-8 col-lg-offset-2 col-lg-8">
        ...
        <table class="table table-striped table-bordered">
          ...
          <% User u = userService.getUserFromCookie(request, response); %>
          <tr>
            <td>Userid:</td>
          ...
          <%
            if (u.getExperimentId() != null) {
              Map<String, Boolean> treatments =
                  experimentService.get(u.getExperimentId()).getTreatments();
              if (treatments != null) {
          ...
          <%
            List<QuizPerformance> results =
                quizPerformanceService.getQuizPerformancesByUser(u.getUserid());
            for (QuizPerformance qp : results) {
          %>
          <tr>
            <td><%= qp.getQuiz() %></td>
            <td>Correct (%)<br>
                <%= qp.getCorrectanswers() %>/<%= qp.getTotalanswers() %>
                (<%= qp.displayPercentageCorrect() %>)
            </td>
            <td>Rank<br>
                <%= qp.getRankScore() %>/<%=qp.getTotalUsers() %>
                (Top-<%= qp.displayRankScore() %>)
            </td>
          </tr>
          <%
            }
