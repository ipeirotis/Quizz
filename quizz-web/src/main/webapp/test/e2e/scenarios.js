describe('quizz e2e test', function() {
  ...
  it ('the table with quizzes should exist', function() {
    var ele = by.className('table');
    expect(ptor.isElementPresent(ele)).toBe(true);
  });

  it ('quiz with name testQuizName should exist', function() {
  ...
  it ('should show a correct summary', function() {
    ...
    expect(progress.getText())
        .toBe('You have answered correctly 10 out of 10 questions.');
  });

  it ('should open the new quiz', function() {
    element(by.buttonText('Click here to start again')).click();
  });

  function nextQuestion(idx) {
    it ('should open question', function() {
      expect(ptor.getCurrentUrl()).toMatch(/\/quiz/);
    });

    it ('should show correct progress', function() {
